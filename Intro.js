...
const introview = ({ item }) => {
    return (
        <View style={style.container}>
            <Image style={{ width: "80%", height: "60%" }} resizeMode="center" source={item.image} />
            <Text style={{ fontSize: RFPercentage(6), fontFamily: "Titan" }}>{item.title}</Text>
            <Text style={{ textAlign: "center", fontFamily: "Rub", fontSize: RFPercentage(2.5), marginTop: 8 }}>{item.word}</Text>
        </View>
    )
}
